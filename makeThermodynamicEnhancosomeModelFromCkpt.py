...
import sys
import argparse
parser = argparse.ArgumentParser(description='Takes a previously learned set of parameters (-res) and a graph archetecture and makes a loadable model containing these..')
...
parser.add_argument('-res',dest='restore',	metavar='<restoreFromCkptFile>',help='Restore parameters from file ', required=True);
...
parser.add_argument('-ac',dest='activityClasses',	metavar='<activityClasses>',help='Include this many activity classes (individual weighted sigmoids, added together) [default=1]', required=False, default="1");
...
args.activityClasses = int(args.activityClasses);
